...
if (!$config) {
    ...
    function autoNumber($tableName, $fieldName, $formatNumber)
    {
        // QUERY INVOICE NUMBER 
        $selectAutoNumber         = "SELECT max($fieldName) AS autoNumber FROM $tableName";
        $queryselectAutoNumber      = mysqli_query($config, $selectAutoNumber);
        $rowselectAutoNumber        = mysqli_fetch_array($queryselectAutoNumber);
        $autoNumber             = $rowselectAutoNumber['autoNumber'];
        $resultautoNumber       = (int) substr($autoNumber, -4, 5);
        $resultautoNumber++;
        $charInvoiceNumber = $formatNumber . date("ymd.");
        $customAutoNumber = $charInvoiceNumber . sprintf("%05s", $resultautoNumber);
