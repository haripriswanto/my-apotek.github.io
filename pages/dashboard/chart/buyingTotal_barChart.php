...
$buyingTotal = "SELECT SUM(total_paid) AS total_purchase,
		...
		WHERE 
			...
			GROUP BY periode
	";
$queryBuyingTotal = mysqli_query($config, $buyingTotal);
?>
<script>
    var ctx = document.getElementById("buyingTotal_barChart_").getContext('2d');
    var myChart = new Chart(ctx, {
        type: 'bar',
        data: {
            labels: [<?php
                        $bulan = '';
                        while ($row0 = mysqli_fetch_array($queryBuyingTotal)) {
                            $bln = $row0['periode'];
                            switch ($bln) {
                                case '' . $yearOption . '-01':
                                    $bulans = 'Januari';
                        ...
                        echo ($bulan);
                        mysqli_data_seek($queryBuyingTotal, 0);
                        ?>],
            datasets: [{
                label: '<?php echo $month . " " . $yearOption; ?>',
                data: [<?php
                        $totalBuying = '';
                        while ($row1 = mysqli_fetch_array($queryBuyingTotal)) {
                            if (!$totalBuying) {
                                $totalBuying = "'" . $row1['total_purchase'] . "'";
                            } else {
                                $totalBuying = $totalBuying . ' , ' . "'" . $row1['total_purchase'] . "'";
                            }
                        };
                        echo ($totalBuying);
                        mysqli_data_seek($queryBuyingTotal, 0);
                ...
                backgroundColor: [<?php
                                    $count1 = 0;
                                    while ($row2 = mysqli_fetch_array($queryBuyingTotal)) {
                                        if ($count1 > 0) echo ',';
                                    ?> 'rgb(8, 129, 204)',
                        'rgb(2, 199, 199)'
                    <?php
                ...
                borderColor: [<?php
                                $count2 = 0;
                                while ($row = mysqli_fetch_array($queryBuyingTotal)) {
                                    if ($count2 > 0) echo ',';
                                ?> 'rgb(6, 98, 156)',
                        'rgb(3, 168, 168)'
                    <?php
...
<canvas id="buyingTotal_barChart_"></canvas>