...
$selectTotalBuyingTransaction = "SELECT count(*) AS total_transaction,
        ...
        AND bl_state <> 'D'
        GROUP BY periode";
// var_dump($selectTotalBuyingTransaction);exit;
$queryTotalBuyingTransaction = mysqli_query($config, $selectTotalBuyingTransaction);

$arrayPeriode = array();
$arrayTotal = array();
while ($data = mysqli_fetch_assoc($queryTotalBuyingTransaction)) {
    $arrayPeriode[] = substr($data['periode'], 8, 2);
    $arrayTotal[]   = $data['total_transaction'];
}
?>
<script>
    var ctx = document.getElementById('buyingTransactionTotal_line').getContext('2d');
...
<canvas id="buyingTransactionTotal_line"></canvas>