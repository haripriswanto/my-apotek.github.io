...
<div class="table-responsive">
  <table class="table table-active table-hover" id="dataCart">
    ...
    <tbody>
      <?php
      $selectDataCart = "SELECT tb_buying_cart.*, tb_master_stock.*, tb_buying_cart.batch_code as batch FROM tb_buying_cart 
      ...
      ?>
        ...
        while ($rowSelectDataCart = mysqli_fetch_array($querySelectDataCart)) {
          ...
          $batch_code             = $rowSelectDataCart['batch'];
...
<script type="text/javascript">
  $("#dataCart").dataTable();
  $('#totalItem').html('<font size="6" v-align="top"><b>' + formatNumber(<?= $total_item_ ?>) + '</b></font>');
  $('#totalHarga').html('<font size="6" v-align="top"><b>' + formatNumber(<?= $total_harga_result ?>) + '</b></font>');
  $('#totalItem_input').val('<?= $total_item_ ?>');
  $('#totalHarga_input').val('<?= $total_harga_result ?>');
  $('#moneyPaid_input').val('<?= $total_harga_result ?>');
</script>