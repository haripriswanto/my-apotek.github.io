...
?>
	...
	<input type="hidden" name="supplier_code" id="supplier_code" value="<?php echo $supplier_code ?>">
	<input type="hidden" name="supplier_name" id="supplier_name" value="<?php echo $supplier_name ?>">
	...
	// Check Stok 
	$selectCheckStok = "SELECT tb_buying_cart.product_name as produk, tb_master_stock.*, tb_buying_cart.* 
		FROM tb_master_stock 
		INNER JOIN tb_buying_cart
		ON tb_master_stock.product_code_relation = tb_buying_cart.product_code_relation 
		WHERE 
		tb_master_stock.product_stock < tb_buying_cart.selling_qty AND
		tb_master_stock.stockable = '1' AND 
		tb_master_stock.bl_state = 'A' AND 
		tb_master_stock.bl_state = 'A' AND 
		tb_master_stock.outlet_code_relation = '$system_outlet_code' AND 
		tb_buying_cart.user_name = '$sessionUser' ";
