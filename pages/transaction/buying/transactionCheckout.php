...
if (!empty($_SESSION['login']['user_name'])) {
	...
	$money_paid			= mysqli_escape_string($config, $_POST['money_paid']);
	...
	$supplier_code		= mysqli_escape_string($config, $_POST['supplier_code']);
	$supplier_name		= mysqli_escape_string($config, $_POST['supplier_name']);
	$quantity 			= mysqli_escape_string($config, $_POST['total_item_']);
	$transactionDate 	= mysqli_escape_string($config, $_POST['transaction_date']);
	$transactionTime 	= mysqli_escape_string($config, $_POST['transaction_time']);
	$updatePrice 		= mysqli_escape_string($config, $_POST['updatePrice']);

	// echo "<script>enableButton();toastr.success('Total Item: " . $total_item_ . "');</script>";
	// exit;
	...
	if ($queryInsertTransaction and $queryInertPayment) {
		...
		while ($rowAdditionsStock = mysqli_fetch_array($queryAdditionsStock)) {
			...
			$btach_code_buying 			= $rowAdditionsStock['batch_code'];
			$exp_date_buying 			= $rowAdditionsStock['exp_date'];

			if ($stockable == 1) {
				$result_stock = $amount_qty1 + $qty_detail;
				$updateStock1 = "UPDATE tb_master_stock
									SET batch_code = '$btach_code_buying', product_stock = '$result_stock', expire_date = '$exp_date_buying', ts_update = '" . date('Y-m-d H:i:s') . "'
...
?>
	...
	<?php
				...
				echo "<script>toastr['error']('Error Query Inert LOG');</script>";
			}
		} else {
			echo "<script>toastr.error('Error queryInsertTransactionDetail !');";
		}
	}
