...
if ($queryUpdateStockHistory && $queryDeleteDetail) {
	...
	if ($queryDeleteTransaction) {
		...
		if ($queryDeletePayment) {
			...
			if ($queryInsertLog) {
				// Notification
				echo "<script>toastr.success('Berhasil Hapus', 'Berhasil hapus Dengan No. " . $invoice_number . "', 'success');LoadReviewTransaction();closeForm();</script>";
			} else {
				echo "<script>toastr.error('Error Query Update Log')</script>";
			}
		} else {
			echo "<script>toastr.error('Error Query Update Payment')</script>";
		}
	} else {
		echo "<script>toastr.error('Error Query Update Transaction')</script>";
	}
} else {
	echo "<script>toastr.error('Error Query Update Drop Stok')</script>";
}
