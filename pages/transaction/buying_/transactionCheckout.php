...
if (!empty($_SESSION['login'])) {
	...
	// echo "<script>toastr.success(nilai total pembelian . $total_harga_);</script>";
	if ($transactionDate == '') {
	...
	// $invoiceNumber = autoNumber('tb_buying_transaction', 'invoice_number', 'BUY-');
	// $queueNumber = autoNumber('tb_buying_transaction', 'queue_number', )

	// QUERY INVOICE NUMBER 
	$selectInvoiceNumber 		= "SELECT max(invoice_number) AS invoice_number FROM tb_buying_transaction";
	...
	// QUERY QUEUE NUMBER 
	$selectQueueNumber 	= "SELECT max(queue_number) AS queue_number FROM tb_buying_transaction";
	...
	if (($total_harga_ < $nominal_cash) or ($total_harga_ = $nominal_cash)) {
			...
			// INSERT TRANSACTION
			$valuesTransaction	= "(id_buying_transaction, invoice_number, queue_number, customer_code_relation, customer_description, dokter_code_relation, dokter_description, total_item, date_insert, time_insert, ts_insert, outlet_code_relation, user_code_relation, ip_address, note, bl_state)
			...
			// INSERT DETAIL
			$valuesDetail	= "(id_buying_transaction_detail, invoice_number_relation, product_code_relation, product_name, product_description, product_qty, unit_code_relation, unit_description, category_code_relation, category_description, 
		id_transaction, transaction_description, buying_price, user_code_relation, outlet_code_relation, ts_insert, bl_state)

		SELECT '" . sha1(generate(20)) . "', '$invoiceNumber', tb_buying_cart.product_code_relation, tb_buying_cart.product_name, tb_buying_cart.product_description, tb_buying_cart.buying_qty, tb_buying_cart.unit_code_relation, tb_buying_cart.unit_description, tb_buying_cart.category_code_relation, tb_buying_cart.category_description, '12', 'BUYING', tb_buying_cart.buying_price, '$sessionUser', '$system_outlet_code', '$currentDate $currentTime', 'A'  
		...
		AND tb_buying_transaction.invoice_number = '$invoiceNumber' ";
			...
			// INSERT PAYMENT
			$valuesPayment	= " (id_buying_payment, invoice_number_relation, type_of_payment, total_cash, total_paid, money_paid, refund, transaction_code, transaction_description, user_code_relation, ip_address, date_insert, time_insert, ts_insert, bl_state)
    		VALUES ('" . sha1(generate(20)) . "', '$invoiceNumber', '$payment_type', '$nominal_cash', '$total_harga_', '$nominal_cash', '$refund', '12', 'BUYING', '$sessionUser', '$ip_address', '$transaction_date', '$transaction_time', '$currentDate $currentTime', 'A')";
	...
	if (($total_harga_ < $nominal_debit) or ($total_harga_ = $nominal_debit)) {
			...
			// INSERT PAYMENT
			$valuesPayment	= " (id_buying_payment, invoice_number_relation, type_of_payment, total_debit, total_paid, money_paid, refund, card_number, card_holder_name, transaction_code, transaction_description, user_code_relation, ip_address, date_insert, time_insert, ts_insert, bl_state)
    		VALUES ('" . sha1(generate(20)) . "', '$invoiceNumber', '$payment_type', '$nominal_debit', '$total_harga_', '$nominal_debit', '$refund', '$card_number', '$name_card', '12', 'BUYING', '$sessionUser', '$ip_address', '$transaction_date', '$transaction_time', '$currentDate $currentTime', 'A')";
	...
	if (($total_harga_ < $nominal_credit) or ($total_harga_ = $nominal_credit)) {
			...
			// INSERT PAYMENT
			$valuesPayment	= " (id_buying_payment, invoice_number_relation, type_of_payment, total_credit, total_paid, money_paid, refund, card_number, card_holder_name, transaction_code, transaction_description, user_code_relation, ip_address, date_insert, time_insert, ts_insert, bl_state)
    		VALUES ('" . sha1(generate(20)) . "', '$invoiceNumber', '$payment_type', '$nominal_credit', '$total_harga_', '$nominal_credit', '$refund', '$card_number', '$name_card', '12', 'BUYING', '$sessionUser', '$ip_address', '$transaction_date', '$transaction_time', '$currentDate $currentTime', 'A')";
	...
	if ($valuesTransaction != '' or $valuesDetail != '' or $valuesPayment != '') {
		...
		$queryInsertTransaction = mysqli_query($config, "INSERT INTO tb_buying_transaction" . $valuesTransaction);
		$queryinsertPayment = mysqli_query($config, "INSERT INTO tb_buying_payment" . $valuesPayment);
		$queryinsertDetail = mysqli_query($config, "INSERT INTO tb_buying_transaction_detail" . $valuesDetail);
        ...
        AND tb_buying_cart.outlet_code_relation = '$system_outlet_code' ");
			...
			while ($rowAdditionsStock = mysqli_fetch_array($queryAdditionsStock)) {
			    ...
			    VALUES ('" . sha1(generate(20)) . "', '$product_code_buying', '$product_name_buying', '0', '$qty_detail', '$result_stock', '12', 'BUYING', '$sessionUser', '$system_outlet_code', '$ip_address', 'Stok Keluar Melalui Penjualan', '$transaction_date', '$transaction_time', '$currentDate $currentTime', 'A')");
				}
			}
			if ($queryUpdateStock) {
				...
				$queryLogInsert = mysqli_query($config, "INSERT INTO log_activity(
				        ...
				        VALUES ( '" . sha1(generate(10)) . "', '$currentDate $currentTime', 'CHECKOUT BUYING', 'Checkout Dengan Invoice " . $invoiceNumber . "' , 'A', '$ip_address', '$sessionUser', '$os', '$browser')");
