...
if (!empty($_SESSION['login'])) {
  ...
  $insert_nama      = mysqli_escape_string($config, $_POST['insert_nama']);
  $insert_birthday  = mysqli_escape_string($config, $_POST['insert_birthday']);
  $insert_gender    = mysqli_escape_string($config, $_POST['insert_gender']);
  $insert_address    = mysqli_escape_string($config, $_POST['insert_address']);
  $insert_email      = mysqli_escape_string($config, $_POST['insert_email']);
  $insert_phone      = mysqli_escape_string($config, $_POST['insert_phone']);
  $insert_username  = mysqli_escape_string($config, $_POST['insert_username']);
  $insert_akses      = mysqli_escape_string($config, $_POST['insert_akses']);
    ...
    if ($insertDataUser) {
      ...
      if (!$result) {
        ...
        echo "<script type='text/javascript'>
                  closeForm();
                  loadDataUser();
                  toastr.success('Berhasil Menambahkan Data User " . $insert_nama . " ', 'success');
                </script>";
        mysqli_close($config);
      }
    }
