...
if (!empty($_SESSION['login'])) {
	...
	$id_user 	= mysqli_escape_string($config, $_POST['idDelete']);
	$full_name	= mysqli_escape_string($config, $_POST['full_name']);
	...
	if (!$result) {
		...
		if (!$queryInsertLogData) {
			...
			echo "<script>
					closeForm();
					loadDataUser();
					toastr.success('Berhasil Hapus Data " . $full_name . "', 'success');
				</script>";
			mysqli_close($config);
		}
	}
